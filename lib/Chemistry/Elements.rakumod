...
The Perl version of C<Chemistry::Elements> was my first module, so
...
# In the following functions $lang is used to declare the language for
# the query/result.  The lang_str_to_int method converts the language
# string to the language index.
    method lang_str_to_column (Str:D $lang --> Int:D) {
        do given $lang {
...
#| number or a ZInt
    method get_name_by_Z (ZInt(Cool) $Z, Str:D $lang = "default" --> Str:D) {
        %names{$Z}[self.lang_str_to_column($lang)];
    }

#|Return the element name (as a Str) by the ChemicalSymbol. You can
#| provide a second argument to choose the language.
    method get_name_by_symbol (
      ChemicalSymbol $symbol, Str:D $lang = "default" --> Str:D
    ) {
